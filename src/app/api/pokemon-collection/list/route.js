...
import { getCollection, getCustomTags } from '@/lib/collectionStorage';
...
export async function GET() {
    ...
    try {
        const [pokemon, customTags] = await Promise.all([
            getCollection(username),
            getCustomTags(username),
        ]);
        return NextResponse.json({ username, pokemon, customTags });
    } catch (err) {
